---
title: "DevContainer+Claude CodeでRDB学習環境を構築した ― 詰まったポイント4つも解説"
emoji: "🐳"
type: "tech"
...
> **追記：** その後 DDD × UUID スキーマへ移行した際に4つ目の詰まりポイントが発生したので末尾に追加しています。
...
### 3. Windows の CRLF 改行コードでシェルスクリプトが動かない
...
### 4. `docker-entrypoint-initdb.d` の初期スキーマが古いまま残り続ける

**状況：** DDD フェーズへ移行して `id` を `SERIAL`（整数）から `UUID` に変更した後、スモークテストが以下のエラーで落ちた。

```
ERROR: invalid input syntax for type integer: "11111111-1111-4111-a111-111111111111"
```

**原因：** PostgreSQL の `docker-entrypoint-initdb.d/` は**データボリュームが空のとき（初回起動時）しか実行されない**。

```
init/01_init_schema.sql  ← ここに古い SERIAL スキーマがあった
```

`docker-compose.yml` で `postgres_data` ボリュームを永続化しているため、`Rebuild Container` してもデータボリュームが残り続け、古い integer の `id` を持つテーブルがそのまま使われ続けていた。

**解決方法①：その場で手動修正（一時的な対処）**

```bash
psql -h postgres -U postgres -d learning \
  -c "DROP SCHEMA public CASCADE; CREATE SCHEMA public;"
psql -h postgres -U postgres -d learning \
  -f /workspace/05_DDD統合/schema.sql
```

**解決方法②：`init/` から古いスキーマを削除して役割を分離する（恒久的な対処）**

`docker-entrypoint-initdb.d/` には拡張機能の有効化のみ残し、テーブル定義は `setup.sh`（`postCreateCommand`）で管理する。

```sql
-- init/01_init_schema.sql（変更後）
-- テーブル定義は setup.sh から 05_DDD統合/schema.sql で行う
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
```

さらに `setup.sh` でスキーマが正しいかを UUID 型チェックで自動検出するようにした。
...
これで次回 `Rebuild Container` 時に古いスキーマが残っていても自動で修正される。

**教訓：** `docker-entrypoint-initdb.d/` はボリューム永続化との相性に注意。スキーマが進化するプロジェクトでは `postCreateCommand` 側で管理し、`init/` は拡張機能など冪等な操作だけに絞るのが安全。
...
| 詰まったポイント | 原因 | 解決方法 |
|---|---|---|
| `runArgs --env-file` が効かない | docker-compose 使用時は非対応 | `docker-compose.yml` に `env_file` を書く |
| Docker ビルドエラー | apt-get にパイプコマンドを混在 | Node.js のインストールを別 RUN に分離 |
| シェルスクリプトが動かない | Windows の CRLF 改行コード | `sed -i 's/\r//'` または `.gitattributes` で解決 |
| 古いスキーマ（integer id）が残り続ける | `docker-entrypoint-initdb.d` は初回起動時のみ実行 | `init/` は拡張機能のみ、テーブルは `postCreateCommand` で管理 + UUID 型チェック |
